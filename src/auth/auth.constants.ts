export const JWT_SECRET = 'JWT_SECRET';
export const JWT_EXPIRES_IN = 'JWT_EXPIRES_IN';
export const RESEND_API_KEY = 'RESEND_API_KEY';
export const DATABASE_URL = 'DATABASE_URL';
export const AUTH_COOKIE_NAME = 'AUTH_COOKIE_NAME';
export const COOKIE_MAX_AGE = 'COOKIE_MAX_AGE';
export const COOKIE_SECURE = 'COOKIE_SECURE';
export const GOOGLE_CLIENT_ID = 'GOOGLE_CLIENT_ID';
export const GOOGLE_CLIENT_SECRET = 'GOOGLE_CLIENT_SECRET';
export const GOOGLE_CALLBACK_URL = 'GOOGLE_CALLBACK_URL';
export const GOOGLE_REDIRECT_URL = 'GOOGLE_REDIRECT_URL';
export const REFRESH_TOKEN_COOKIE_NAME = 'REFRESH_COOKIE_NAME';
export const JWT_REFRESH_EXPIRES_IN = 'JWT_REFRESH_EXPIRES_IN';
export const REFRESH_TOKEN_MAX_AGE = 'REFRESH_TOKEN_MAX_AGE';
